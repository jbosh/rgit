...
namespace rgit
{
    ...
    {
        private static readonly bool g_darkModeSupported = false;
        private static readonly uint g_buildNumber;

        private static bool g_darkModeEnabled = false;
        ...
        static DarkMode()
        {
            ...
            var _GetIsImmersiveColorUsingHighContrast = GetProcAddress(hUxtheme, 106);
            ...
            g_darkModeEnabled = ShouldAppUseDarkCallback() && !IsHighContrast();
        ...
        private enum PreferredAppMode : int
        {
            Default,
            AllowDark,
            ForceDark,
        ...
        static extern bool SystemParametersInfo(uint uiAction, uint uiParam, ref HIGHCONTRASTW pvParam, uint fWinIni); // T = any type
        ...
        [DllImport("User32.dll", SetLastError = true)]
        static extern nint SetWindowsHookExA(int idHook, WndProcDelegate lpfn, IntPtr hmod = default, ushort dwThreadId = 0);

        [DllImport("User32.dll", SetLastError = true)]
        static extern nint CallNextHookEx(IntPtr hhk, int nCode, nuint wParam, nuint lParam);

        [DllImport("User32.dll", SetLastError = true)]
        static extern nint UnhookWindowsHookEx(IntPtr hook);

        [DllImport("kernel32.dll")]
        static extern bool VirtualProtectEx(IntPtr hProcess, IntPtr lpAddress, UIntPtr dwSize, uint flNewProtect, out uint lpflOldProtect);

        [DllImport("User32.dll")]
        static extern ushort GetWindowThreadProcessId(IntPtr hWnd, out ushort lpdwProcessId);

        [DllImport("kernel32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
        static extern IntPtr GetModuleHandleA(string? lpModuleName);

        [DllImport("User32.dll", SetLastError = true)]
        static extern nint SendMessageA(IntPtr hWnd, uint Msg, nuint wParam, nuint lParam);
        ...
        public delegate IntPtr WndProcDelegate(int nCode, nuint wParam, nuint lParam);


        public class GCFuncHandle : IDisposable
        {
            ...
            public static T CreateDelegate<T>(T d, out GCFuncHandle handle) where T : Delegate
            {
            ...
            protected override IntPtr WndProc(IntPtr hWnd, uint msg, IntPtr wParam, IntPtr lParam)
            {
                switch ((WindowsMessage)msg)
                {
                    case WindowsMessage.WM_CREATE:
                    {
                        ...
                        {
                            AllowDarkModeForWindowCallback(hWnd, g_darkModeEnabled);
                            SetDarkTheme(hWnd);
